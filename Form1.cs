...
namespace Commivoyager
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                richTextBox1.Text = "Оптимальный маршрут: " + result[result.Count - 1] + "->" + result[0] + " ";
                for (int i = 0; i < result.Count; i += 2)
                {
                    int element = result[i];
                    richTextBox1.Text += result[i] + "->" + result[i + 1] + " ";
                }
        ...
        private void FillDataGridView(DataGridView dataGridView)
        {
            ...
            // Заполняем все ячейки, кроме диагонали
            for (int row = 0; row < rowCount; row++)
            {
                for (int column = 0; column < columnCount; column++)
                {
                    if (row != column) // Пропускаем диагональные ячейки
                    {
                        int randomNumber = random.Next(101); // Генерируем случайное число от 0 до 100
                        dataGridView.Rows[row].Cells[column].Value = randomNumber; // Устанавливаем значение в ячейку
                    }
                }
            }
        }
